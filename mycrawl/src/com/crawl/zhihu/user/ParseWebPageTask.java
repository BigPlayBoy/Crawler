...
import com.crawl.util.JDBCTools;
...
//			Connection cn = ConnectionManage.getConnection();
			...
			if(doc.select("title").size() != 0){
				...
				if(ZhuhuDAO.insetToDB(connection,u)){
...
//		System.out.println(doc.select(".gender").first().child(0).attr("class"));
		if(doc.select(".gender").first().child(0).attr("class").contains("male")){
			u.setSex("男");
		}else {
			u.setSex("女");
		}
		u.setAsks(Integer.parseInt(doc.select(".profile-navbar ").first().child(1).child(0).text()));//提问数
		u.setAnswers(Integer.parseInt(doc.select(".profile-navbar").first().child(2).child(0).text()));		//回答数
		u.setPosts(Integer.parseInt(doc.select(".profile-navbar").first().child(3).child(0).text()));		//文章数
		System.out.println(doc.select(".profile-navbar ").first().child(0).attr("href"));
		try {
			u.setUsername(doc.select(".title-section").first().child(0).text());//用户名
			u.setUrl( doc.select(".profile-navbar ").first().child(0).attr("href"));//用户首页链接
		...
		try {
			u.setHashId(doc.select(".zm-profile-section-list button").first().attr("data-id"));
		}catch (NullPointerException e){
			e.printStackTrace();
			//解析我的主页时，会出现空指针
			u.setHashId("464b39c0a4a0e9ad750da8c859648ed4");
		}
...
//		HttpClientUtil.getWebPage(zh.getHttpClient(),zh.getContext(),gRequest,"utf-8",true);
		File file=new File("src/resources/11.html");
...
//		ParseWebPageTask.p
		ParseWebPageTask parseWebPageTask=new ParseWebPageTask();
		parseWebPageTask.parseUserdetail(document);
	}
}
